...
public class ComboDishService {
    ...
    public ComboDishFullDTO createComboDish(int comboId, int dish1Id, int dish2Id) {
        ComboEntity combo = comboJPA.findById(comboId)
                .orElseThrow(() -> new NoSuchElementException("Combo not found"));

        DishEntity dish1 = dishJPA.findById(dish1Id)
                .orElseThrow(() -> new NoSuchElementException("Dish 1 not found"));

        DishEntity dish2 = dishJPA.findById(dish2Id)
                .orElseThrow(() -> new NoSuchElementException("Dish 2 not found"));
    ...
    public void deleteComboDishByDishId(int comboDishId) {
        ComboDishEntity entity = comboDishJPA.findById(comboDishId)
            .orElseThrow(() -> new NoSuchElementException("ComboDish not found"));
    ...
    public List<ComboDishFullDTO> getComboDishFullDTOs() {
        ...
        return comboDishes.stream()
                ...
                .map(entry -> {
                    ...
                    ComboDishEntity dish2 = entry.getValue().size() > 1 ? entry.getValue().get(1) : null;
                    Integer dish2Id = (dish2 != null && dish2.getDish() != null) ? dish2.getDish().getId() : null;
                    String dish2Name = (dish2 != null && dish2.getDish() != null) ? dish2.getDish().getName() : null;
                    return new ComboDishFullDTO(
                            dish1.getId(),
                            dish1.getCombo().getId(),
                            dish1.getCombo().getName(),
                            dish1.getDish().getId(),
                            dish1.getDish().getName(),
                            dish2Id,
                            dish2Name
                    );
                })
